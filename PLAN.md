...
**Written 2026-09-04 from what the owner actually described.** This is the whole
plan. Nothing older than it is planned from.

**This repository is the app's home.** Phase 0 created it; every phase from 2 on
is built here.
...
This repository, created 2026-09-04, deploying to
`https://bill6006.github.io/life-command/preview/`. The check-in, the record
store, the tests and the deployment pipeline are all here. Nothing older is
planned from.
...
**Old data: start clean.** The owner chose not to import readings from any
earlier app. The state record begins here.
...
what to open on your phone and what to look at. It keeps the decision log and writes
...
Phases are numbered 0 to 9 and nothing else. `docs/NEXT_PROMPT.md` names the
next one on a `Phase:` line. Builders keep a decision register in
`docs/DECISION_LOG.md`; the owner never needs to read it.
